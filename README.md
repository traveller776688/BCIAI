...
- `artifact_audit_fast.py` — 伪迹剔除快速审计（论文 VI-E 末段）：数据级 P1–P3 + 4 被试 LOSO 清洗对比
- `paper/` — 论文源文件（中英）与 PDF
- `results/` — 实验缓存（JSON，协议标记见各文件内 key）

## 运行

依赖 PyTorch (CUDA)、scikit-learn；伪迹剔除（`--artifact-clean`）额外需要 MNE + mne-icalabel。
数据为 SEED-IV 原始 EEG（62 通道），放置路径见
...
# 伪迹剔除臂（默认关闭；开启后缓存键带 _clean 后缀，清洗缓存落 results/artifact_clean_cache/）
python eeg_v5_coupling_experiment.py --fast 8 --artifact-clean
