...
**修订说明（中文版保留）。** 本次与英文版同步修订，一处无法核实的引用已从正文与参考文献中移除。原条目按未完成标注保留如下备查，待核实后决定是否恢复：Suo and Li, "Protocol evaluation of direct-transfer EEG emotion decoding," preprint, 2026（详情待核）。另，伪迹剔除快速审计（n = 4，ICA + ICLabel）已完成，结果写入第 VI-E 节、局限（vi）与未来工作（4）。
